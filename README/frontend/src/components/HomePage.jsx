...
function EnglishPreview() {
  const phrases = ['Professional tone ✅', 'Grammar corrected ✅', 'Confidence score: 92%'];
...
const HomePage = () => {
  ...
  const learningCards = [
    ...
    { 
      ...
      action: () => navigate('/learn-programming') 
    },
    { 
      id: 3, 
      ...
      <div className="home-layout">
        ...
        <aside className="home-sidebar">
          <div className="sidebar-section">
            ...
            <SidebarItem icon={<Bot size={18} />} label="AI Study Buddy" badge="AI" active={chatOpen} onClick={() => setChatOpen(!chatOpen)} />
        ...
        <main className="home-main">
          ...
          <section className="section-block">
            ...
            <div className="learning-grid">
              {learningCards.map((card, i) => (
                <motion.div key={card.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.08 }} className="learning-card" onClick={card.action}>
                  <div className="ev2-module-top" style={{ marginBottom: '16px' }}>
                    <div className="card-icon" style={{ background: `${card.color}15`, color: card.color, marginBottom: 0 }}>{card.icon}</div>
                    <span className="ev2-module-tag" style={{ color: card.tagColor, background: card.tagBg, border: `1px solid ${card.tagColor}30` }}>{card.tag}</span>
                  </div>
                  <h3>{card.title}</h3>
                  <p className="card-desc">{card.desc}</p>
                  <div className="card-preview-wrapper" style={{ marginTop: '1.5rem', marginBottom: '1rem' }}>
                    <card.Preview />
                  </div>
                  <button className="btn-glow-sm" style={{ '--glow-color': card.color }}>{card.buttonText} <ArrowRight size={14} /></button>
                </motion.div>
              ))}
            </div>
          </section>
          ...
          {/* Stats Row */}
          <section className="ev2-section">
            <div className="ev2-stats-grid">
              {EV2_STATS.map(s => (
                <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="ev2-stat-card">
                  <div className="ev2-stat-emoji">{s.icon}</div>
                  <div className="ev2-stat-num">{s.num}</div>
                  <div className="ev2-stat-label">{s.label}</div>
                </motion.div>
              ))}
            </div>
          </section>
          ...
          {/* How It Works */}
          <section className="ev2-section">
            <div className="ev2-section-header">
              <div className="ev2-eyebrow">🗺️ The journey</div>
              <h2 className="ev2-section-title">How Elevate <span className="ev2-text-gradient">works</span></h2>
            </div>
            <div className="ev2-steps-grid">
              {EV2_STEPS.map((s, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="ev2-step">
                  <div className="ev2-step-circle">
                    {s.icon}
                    <div className="ev2-step-num">{i + 1}</div>
                  </div>
                  <div className="ev2-step-label">{s.label}</div>
                  <div className="ev2-step-sub">{s.sub}</div>
                </motion.div>
              ))}
            </div>
          </section>
          ...
          {/* CTA Banner */}
          <section className="ev2-section">
            <div className="ev2-cta-banner">
              <div className="ev2-cta-orb ev2-cta-orb-1" />
              <div className="ev2-cta-orb ev2-cta-orb-2" />
              <div className="ev2-cta-content">
                <div style={{ fontSize: '3rem', marginBottom: 12 }}>🚀</div>
                <h2>Ready to land your dream job?</h2>
                <p>Join engineers who stopped guessing and started practising smarter. Your next offer is one session away.</p>
                <button className="ev2-cta-btn" onClick={() => navigate('/login')}>
